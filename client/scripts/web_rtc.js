...
function solicitar_llamada(btf,room_bthx){
  ...
  if(btf.style.backgroundColor=="green"){
    ...
    socket_client.emit("solicitar llamada",
               {room_rtc:room_bthx});
  }//else
}//solicitar llamada
...
console.log("solicitud de aceptacion de la llamada")
  if(dt.sktid_mnd!=socket_client.id){
    ...
    nudiv.setAttribute("class","flex_row pos_abs top_10 w bac_col_whi col_gre");
...
function cerrar_llamada(roomf){
 console.log("cierra llamada:"+roomf)
  ...
  if(dv_esp_call){
    socket_client.emit("cancelar llamada entrante",
              {room_rtc:roomf});
...
function aceptar_llamada(roomf){
  ...
  socket_client.emit("correr llamada",
              {roomrtc: roomf});
}//aceptar_llamada
