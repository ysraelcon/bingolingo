...
var o_rooms_user= {};//c_rooms_user , collection_rooms_user
//o_rooms_user{roomf:{user_idf:firstname}}
...
for( var room in o_rooms_user)
{
io.to(socket.id).emit("actualizar rooms",
 {o_rooms_user: o_rooms_user[room], room: room});
...
if(!o_rooms_user[roomf])
{
o_rooms_user[roomf]= {};
}//if indef
//console.log(o_rooms_user[roomf])
o_rooms_user[roomf][socket.request.user._id]= socket.request.user.firstname;
socket.join(roomf);
...
{
o_rooms_user: o_rooms_user[roomf],
...
else if(o_msgx.type_room === "private")
{
Chat.findOne({_id: "5a36957e2659be546185f785"}, function(err, chatf)
{
var con_sav=socket.request.user.firstname+": "+o_msgx.msg;
if(chatf.chats_prv[o_msgx.room].length < 15)
{
chatf.chats_prv[o_msgx.room].push(con_sav);
}else
{
chatf.chats_prv[o_msgx.room].shift();
chatf.chats_prv[o_msgx.room].push(con_sav);
}//else shift and push    
chatf.markModified("chats_prv."+o_msgx.room); 
chatf.save((err)=>
{
if(err) throw err;
});//save  
...
//console.log(o_rooms_user[roomf]);
if(o_rooms_user[roomf])
delete o_rooms_user[roomf][socket.request.user._id];
  
io.to(roomf).emit('recibir usuarios en el room',
 {o_rooms_user: o_rooms_user[roomf], room: roomf});
/*
...
if(!o_rooms_user[room_secretf])
{
o_rooms_user[room_secretf]={};
}//if indef
//console.log(o_rooms_user[room_secretf])
o_rooms_user[room_secretf][socket.request.user._id]= socket.request.user.firstname;
...
{
o_rooms_user: o_rooms_user[room_secretf],
skt_id: socket.id,
room: room_secretf,
chat_room: []
});  
...
if(!o_rooms_user[room_bth])
{
o_rooms_user[room_bth]= {};
}//if indef
//o_rooms_user{room_nme{user_id:firstname,...},...}
o_rooms_user[room_bth][socket.request.user._id]= socket.request.user.firstname;
...
o_rooms_user[o_userx.room_bth][socket.request.user._id]= socket.request.user.firstname;
...
for(var ele in o_rooms_user[room_bth])
{
//ele - user_id
m_names.push(o_rooms_user[room_bth][ele])// [firstname, skt_id]
}//for
var o_user= {};
...
{
o_rooms_user: o_rooms_user[room_bth],
skt_id: null,
room: room_bth,
chat_room: chatprv,
prv: "privado"
...
socket.on("crear juego", function(obj_gamex)
{
//obj_gamex{type_game,list_word,nro_player}
console.log("on: crear juego");
...
io.to(socket.id).emit("ver puerta de juego",
{
nro_game: nro_game,
type_game: obj_gamex.type_game,
...
socket.on('tocar puerta de juego', function(obj_gamex)
{
//obj_gamex{nro_game,type_game,list_word,nro_player}
console.log("on: tocar puerta de juego");
console.log(obj_gamex);
...
console.log("nro jugadoresss:"+nro_in_game) 
...
io.to(socket.id).emit("crear room de juego", {
...
if(!jue[obj_gamex.nro_game].en_curso)
{
cuenta_regresiva(obj_gamex.nro_game, jue[obj_gamex.nro_game].player_trn);
}//if
  
jue[obj_gamex.nro_game].en_curso= true;
...
});//skon tocar puerta de juego
 
//los tiempos se manejan del lado del server.  
  
  
  
socket.on("pedir usuarios al room de juego", function(o_gamex)
         {
  
  var usergame= jue[o_gamex.nro_game].nro_player_act;
  
io.to(o_gamex.nro_game).emit("manda user al juego",
{
users_jue: usergame,
nro_game: o_gamex.nro_game,
type_game: o_gamex.type_game,
list_word: o_gamex.list_word,
nro_player: o_gamex.nro_player
});  
  
})//skon pedir usuarios al room de juego
...
if(Object.keys(jue[roomjf].nro_player_act).length)
{jue[roomjf].en_curso= false}
var winner= "";
...
var pmr_id= Object.keys(jue[roomjf].nro_player_act)[ jue[roomjf].player_trn
 % Object.keys(jue[roomjf].nro_player_act).length ];
...
var pmr_id= Object.keys(jue[o_msg_gamex.nro_game].nro_player_act)[ jue[o_msg_gamex.nro_game].player_trn 
 % Object.keys(jue[o_msg_gamex.nro_game].nro_player_act).length ];
...
for(var room in o_rooms_user)
{
if(o_rooms_user[room][socket.request.user._id]){
if(!socket.id) return;
delete o_rooms_user[room][socket.request.user._id];
